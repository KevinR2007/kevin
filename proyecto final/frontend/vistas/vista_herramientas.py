...
from modelos.herramienta import Herramienta
...
def generar_codigo_auto(lista):
    ...
    for item in lista:
        try:
            numeros.append(int(item["codigo"]))
        except:
            pass
...
class VistaHerramientas(Frame):
    ...
    def __init__(self, parent):
        ...
        self.codigo_entry = Entry(form_frame, textvariable=self.herramienta.codigo_var, state="readonly")
        self.codigo_entry.grid(row=0, column=1)

        Label(form_frame, text="Nombre").grid(row=1, column=0, padx=5, pady=5)
        Entry(form_frame, textvariable=self.herramienta.nombre_var).grid(row=1, column=1)

        Label(form_frame, text="Categoría").grid(row=2, column=0, padx=5, pady=5)
        Entry(form_frame, textvariable=self.herramienta.categoria_var).grid(row=2, column=1)

        Label(form_frame, text="Ubicación").grid(row=3, column=0, padx=5, pady=5)
        Entry(form_frame, textvariable=self.herramienta.ubicacion_var).grid(row=3, column=1)

        Label(form_frame, text="Estado").grid(row=4, column=0, padx=5, pady=5)
        Entry(form_frame, textvariable=self.herramienta.estado_var).grid(row=4, column=1)
    ...
    def validar(self):
        if not self.herramienta.nombre_var.get().strip():
            return "El nombre es obligatorio."
        if not self.herramienta.categoria_var.get().strip():
            return "La categoría es obligatoria."
        if not self.herramienta.ubicacion_var.get().strip():
            return "La ubicación es obligatoria."
        if not self.herramienta.estado_var.get().strip():
    ...
    def crear_herramienta(self):
        ...
        nueva = {
            "codigo": self.herramienta.codigo_var.get(),
            "nombre": self.herramienta.nombre_var.get(),
            "categoria": self.herramienta.categoria_var.get(),
            "ubicacion": self.herramienta.ubicacion_var.get(),
            "estado": self.herramienta.estado_var.get()
        }
    ...
    def editar_herramienta(self):
        ...
        # ---------------------- API: actualizar ----------------------
        api_actualizar_herramienta(codigo, data)
        # ------------------------------------------------------------
    ...
    def limpiar_formulario(self):
        self.herramienta.nombre_var.set("")
        self.herramienta.categoria_var.set("")
        self.herramienta.ubicacion_var.set("")
        self.herramienta.estado_var.set("")

        # GENERA NUEVO CÓDIGO (BASADO EN LISTA DEL BACKEND)
        nuevo = generar_codigo_auto(self.lista_herramientas)
        self.herramienta.codigo_var.set(nuevo)
    ...
    def seleccionar_fila(self, event):
        ...
        self.herramienta.codigo_var.set(datos["codigo"])
        self.herramienta.nombre_var.set(datos["nombre"])
        self.herramienta.categoria_var.set(datos["categoria"])
        self.herramienta.ubicacion_var.set(datos["ubicacion"])
        self.herramienta.estado_var.set(datos["estado"])

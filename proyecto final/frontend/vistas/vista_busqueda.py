...
from modelos.busqueda import Busqueda
...
class VistaBusqueda(Frame):
    ...
    def __init__(self, parent, vista_herramientas, vista_prestamos):
        ...
        Entry(filtro_frame, textvariable=self.busqueda.entrada_var, width=40).grid(row=0, column=1, padx=6)
    ...
    # --------------------------
    def buscar(self):
        termino = self.busqueda.entrada_var.get().strip().lower()
        if not termino:
            self.mostrar_todo()
            return
